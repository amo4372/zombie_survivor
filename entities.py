...
class Player:
    ...
    def level_up(self):
        ...
        self.skill_cards = self.skill_tree.get_random_skill_cards(3)
...
class Enemy:
    ...
    def update(self, dt, player_x, player_y, player):
        ...
        if self.grappled:
            ...
            dist = math.hypot(dx, dy)
            if dist > 50:
                self.x += (dx / dist) * self.speed * 3 * dt * 60
                self.y += (dy / dist) * self.speed * 3 * dt * 60
            else:
                self.grappled = False
            return
    ...
    def _boss_behavior(self, dt, player_x, player_y, dist, player):
        ...
        if self.enemy_type == EnemyType.BOSS_LONG:
            ...
            # 普通向玩家移动
            if dist > 0:
                self.x += (player_x - self.x) / dist * self.speed * dt * 60
                self.y += (player_y - self.y) / dist * self.speed * dt * 60
